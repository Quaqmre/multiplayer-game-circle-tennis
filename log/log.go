...
type Logger interface {
	...
	Print(v ...interface{})
	//Println wirtes a line to the log
	Println(v ...interface{})
...
//NewStdLogger creates a *log.Logger("log" is from the Goo standard library)
//that forwards messages to the provided upspin logger using a lobBridge.The
//standard logger is configured with log.Lshortfile , this log line
//format which is parsed to extract the log message (skipping the filename,line number) to forward it to the provided upspin logger.
func NewStdLogger(l Logger) *log.Logger {
	lb := logBridge{l}
	return log.New(lb, "", log.Lshortfile)
}

//Register connects an ExternalLogger to ht default logger.This may only be called once.
...
var _ Logger = (*logger)(nil) // check logger implement all Logger interface methods ??
